...
| Tool | Purpose | Status |
|------|---------|--------|
| Intel PIN 4.1 | Binary instrumentation | Required (symlink) |
| ZSim | Workload tracing | Integrated |
...
### Intel PIN 4.1 (Binary Instrumentation)
- Required for ZSim-based workload execution
- Location: `external/pin/` (symlink to downloaded PIN 4.1)
- Setup: Download and symlink:
  ```bash
  cd external
...
### ZSim (Workload Tracing)
- Fast x86-64 simulator for multi-core systems
- Instruction-level tracing via PIN
- Location: `external/zsim/`
...
└── external/
    ├── pin/                  # Intel PIN 4.1 (symlink)
    ├── zsim/                 # ZSim workload tracer
