...
// PE's row range inside the offload region via a plain loop (no pthreads), so all
// work is attributed to one device context rather than N PEs.
static void device_kernel(void* raw) {
    SpMVData* data = (SpMVData*)raw;
    for (int i = 0; i < data->num_device_pes; i++) {
