...
// Per-PE thread argument
struct PEThreadArg {
    int pe_id;
    SharedReductionData* shared;
};

// One thread per device PE: each PE reduces its own chunk concurrently.
// Under the cosim harness each device-domain thread is mapped (round-robin)
// to a distinct device PE core, so all PEs accrue cycles (not just PE-0).
static void* pe_thread(void* a) {
    PEThreadArg* arg = (PEThreadArg*)a;
    DeviceReducer pe(arg->pe_id, *arg->shared);
...
static void device_kernel(void* raw) {
    ...
    std::vector<PEThreadArg> args(n);
    for (int i = 0; i < n; i++) {
        args[i].pe_id = i;
        args[i].shared = shared;
        pthread_create(&tids[i], nullptr, pe_thread, &args[i]);
    }
    for (int i = 0; i < n; i++) {
