...
def parse_benchmark_output(filepath: str) -> Dict:
    ...
    for line in lines:
        ...
        workload_match = re.match(r'# ([^:]+):', line)
        if workload_match:
            current_workload = workload_match.group(1).strip()
            continue

        # Bank configuration
        bank_match = re.match(r'(\w+) - (Bank\d+-\d+KB)', line)
        if bank_match:
            current_bank = bank_match.group(2)
            continue

        # Programming model and topology
        if '--- Message Passing - Baseline' in line or '--- Message Passing - H-tree' in line:
            # Save previous result if exists
            if current_workload and current_bank and current_model and current_topology and current_data:
        ...
        if '--- Message Passing - LIBCom' in line:
            # Save previous result if exists
            if current_workload and current_bank and current_model and current_topology and current_data:
                key = (current_workload, current_bank, current_model, current_topology)
                results[key] = current_data
        ...
        if '--- Shared Memory - Baseline' in line or '--- Shared Memory - H-tree' in line:
            # Save previous result if exists
            if current_workload and current_bank and current_model and current_topology and current_data:
                key = (current_workload, current_bank, current_model, current_topology)
                results[key] = current_data
        ...
        if '--- Shared Memory - LIBCom' in line:
            # Save previous result if exists
            if current_workload and current_bank and current_model and current_topology and current_data:
                key = (current_workload, current_bank, current_model, current_topology)
                results[key] = current_data
        ...
        # Check if we're in a results section
        if 'Results' in line and ('MESSAGE PASSING' in line or 'SHARED MEMORY' in line):
        ...
        if current_workload and current_bank and current_model and current_topology:
            # Total cycles
            cycles_match = re.search(r'Total cycles:\s+(\d+)', line)
            if cycles_match:
                current_data['total_cycles'] = int(cycles_match.group(1))

            # Compute cycles
            compute_match = re.search(r'Compute cycles:\s+(\d+)', line)
            if compute_match:
                current_data['compute_cycles'] = int(compute_match.group(1))

            # Transfer cycles
            transfer_match = re.search(r'Transfer cycles:\s+(\d+)', line)
            if transfer_match:
                current_data['transfer_cycles'] = int(transfer_match.group(1))

            # Total energy
            energy_match = re.search(r'Total energy \(relative\):\s+([\d.]+)', line)
            if energy_match:
                current_data['total_energy'] = float(energy_match.group(1))

            # Transfers
            transfers_match = re.search(r'Inter-subarray (?:block )?transfers:\s+(\d+)', line)
            if transfers_match:
                current_data['num_transfers'] = int(transfers_match.group(1))
...
def main():
    if len(sys.argv) < 2:
        print("Usage: analyze_all_workloads_v2.py <results_file>")
        sys.exit(1)
