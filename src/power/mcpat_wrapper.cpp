...
void McPATWrapper::setNoCActivity(const NoCActivityStats& stats) {
    ...
    std::cout << "[McPATWrapper] NoC activity updated from Garnet:" << std::endl;
    std::cout << "  Packets: " << stats.total_packets << std::endl;
    std::cout << "  Flits: " << stats.total_flits << std::endl;
    std::cout << "  Hops: " << stats.total_hops << std::endl;
    std::cout << "  Buffer R/W: " << stats.buffer_reads << "/" << stats.buffer_writes << std::endl;
    std::cout << "  Crossbar traversals: " << stats.crossbar_traversals << std::endl;
    std::cout << "  Link traversals: " << stats.link_traversals << std::endl;
}
...
void McPATWrapper::extractResults() {
    ...
    // NoC power - activity-based calculation using Garnet stats
    // Based on McPAT's router model (cacti/router.h):
    // - Router = arbiter + crossbar + buffers
    // - Link energy per bit
    PowerMetrics noc_power;
    if (config_.has_noc) {
        // Technology and frequency scaling
        double tech_scale = std::pow(90.0 / config_.tech_node_nm, 2);  // Quadratic scaling
        double freq_ghz = config_.noc_clock_mhz / 1000.0;

        // Per-component energy (nJ) - derived from McPAT/CACTI router model at 22nm
        double energy_per_buffer_access_nj = 0.02 * tech_scale;   // VC buffer read/write
        double energy_per_crossbar_nj = 0.05 * tech_scale;        // Crossbar traversal
        double energy_per_arbiter_nj = 0.01 * tech_scale;         // Arbiter decision
        double energy_per_link_bit_nj = 0.0001 * tech_scale;      // Link per bit

        // Calculate dynamic energy from Garnet activity
        double buffer_energy_nj = (noc_activity_.buffer_reads + noc_activity_.buffer_writes)
                                  * energy_per_buffer_access_nj;
        double crossbar_energy_nj = noc_activity_.crossbar_traversals * energy_per_crossbar_nj;
        double arbiter_energy_nj = noc_activity_.arbiter_events * energy_per_arbiter_nj;
        double link_energy_nj = noc_activity_.link_traversals
                                * config_.noc_flit_size_bits * energy_per_link_bit_nj;

        double total_dynamic_energy_nj = buffer_energy_nj + crossbar_energy_nj
                                         + arbiter_energy_nj + link_energy_nj;

        // Convert to power (W) = Energy (J) / Time (s)
        double sim_time_s = 0.0;
        if (noc_activity_.total_cycles > 0 && noc_activity_.clock_mhz > 0) {
            sim_time_s = noc_activity_.total_cycles / (noc_activity_.clock_mhz * 1e6);
        } else if (total_cycles_ > 0 && config_.noc_clock_mhz > 0) {
            sim_time_s = total_cycles_ / (config_.noc_clock_mhz * 1e6);
        }

        if (sim_time_s > 0) {
            noc_power.runtime_dynamic = (total_dynamic_energy_nj * 1e-9) / sim_time_s;
        } else {
            // Fallback: estimate based on activity rate
            noc_power.runtime_dynamic = total_dynamic_energy_nj * 1e-9 * freq_ghz;
        }

        // Leakage power (per router, scales with area)
        double leakage_per_router_w = 0.01 * tech_scale;  // ~10mW per router at 22nm
        noc_power.subthreshold_leakage = leakage_per_router_w * config_.noc_num_routers * 0.7;
        noc_power.gate_leakage = leakage_per_router_w * config_.noc_num_routers * 0.3;
        noc_power.total_leakage = noc_power.subthreshold_leakage + noc_power.gate_leakage;

        noc_power.total_dynamic = noc_power.runtime_dynamic;
        noc_power.total_power = noc_power.total_dynamic + noc_power.total_leakage;

        // Store energy for queries
        noc_power.total_energy = total_dynamic_energy_nj;

        std::cout << "[McPATWrapper] NoC power calculated from Garnet activity:" << std::endl;
        std::cout << "  Buffer energy: " << buffer_energy_nj << " nJ" << std::endl;
        std::cout << "  Crossbar energy: " << crossbar_energy_nj << " nJ" << std::endl;
        std::cout << "  Arbiter energy: " << arbiter_energy_nj << " nJ" << std::endl;
        std::cout << "  Link energy: " << link_energy_nj << " nJ" << std::endl;
        std::cout << "  Dynamic power: " << noc_power.runtime_dynamic << " W" << std::endl;
        std::cout << "  Leakage power: " << noc_power.total_leakage << " W" << std::endl;
    }
