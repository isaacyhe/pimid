...
void MESIBottomCC::processInval(Address lineAddr, uint32_t lineId, InvType type, bool* reqWriteback) {
    MESIState* state = &array[lineId];
    if (*state == I) return;  // already invalidated (QEMU race)
    switch (type) {
...
uint64_t MESITopCC::sendInvalidates(Address lineAddr, uint32_t lineId, InvType type, bool* reqWriteback, uint64_t cycle, uint32_t srcId) {
    ...
    if (!e->isEmpty()) {
        ...
        // assert(sentInvs == e->numSharers);  // May differ under QEMU races
        if (type == INV) {
            e->numSharers = 0;
        } else {
    ...
    switch (type) {
        case PUTX:
            if (!e->sharers[childId]) {
                /* QEMU race: sharer already cleared by a concurrent INV. */
                *childState = I;
                break;
            }
        ...
        case GETS:
            if (e->isEmpty() && haveExclusive && !(flags & MemReq::NOEXCL)) {
                ...
                *childState = E;
                ...
                /* else: already a sharer (QEMU race), no-op */
