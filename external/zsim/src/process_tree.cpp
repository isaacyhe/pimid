...
void ProcessTreeNode::heartbeat() {
    ...
    // PIMID: the heartbeats stat counter is disabled (see init.cpp). Guard the
    // increment so this stays safe if a workload ever issues the HEARTBEAT
    // magic op; the dump-trigger logic below is unaffected.
    if (zinfo->profHeartbeats) zinfo->profHeartbeats->atomicInc(procIdx);
