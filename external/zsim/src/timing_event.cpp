...
void CrossingEvent::simulate(uint64_t simCycle) {
    if (!called) {
        ...
        if (!called) { //have to check again, AFTER reading the cycles! Otherwise, we have a race
            ...
            simCount++;
#endif
            // Reset numParents before requeue: at this point simulate() is
            // running so all parents have already fired. The nonzero count is
            // stale bookkeeping; requeue() asserts numParents == 0.
            numParents = 0;
            requeue(nextCycle);
            return;
        }
