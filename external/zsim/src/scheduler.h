...
class Scheduler : public GlobAlloc, public Callee {
    ...
    public:
        ...
        void finish(uint32_t pid, uint32_t tid) {
            ...
            if (th->fakeLeave) {
                finishFakeLeave(th);
                leaveLocked(pid, tid, th->cid);
            }

            // The normal sequence is leave, finish. If the thread is still
            // RUNNING (e.g. during fast-forward), do the leave while holding
            // the lock to avoid races with concurrent state modifications.
            if (th->state == RUNNING) {
        ...
        // Core leave logic, caller must hold schedLock
        void leaveLocked(uint32_t pid, uint32_t tid, uint32_t cid) {
            uint32_t gid __attribute__((unused)) = getGid(pid, tid);
