...
void ProcStats::notifyDeschedule() {
    // Only update at phase boundaries to avoid reading inconsistent counter
    // values from cores that are still running mid-phase. In non-overcommitted
    // systems (PIMID's model: 1 process, #threads <= #cores), descheduling
    // only happens at phase boundaries, so this is always safe.
    if (zinfo->numPhases > lastUpdatePhase) {
        update();
    }
}
