...
public:
    ...
    // Message size in BITS (divided by link bitWidth to get flit count).
    // Configurable via m_control_msg_size and m_data_msg_size.
    // Defaults: Control = 64b (1 flit), Data = 576b (5 flits) for 128-bit links.
    // For PIM in-memory networks, set data to link width (1 flit).
...
inline uint32_t Network::m_control_msg_size = 64;   // 8 bytes in bits
inline uint32_t Network::m_data_msg_size = 576;     // 72 bytes in bits
