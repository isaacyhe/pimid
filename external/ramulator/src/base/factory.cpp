...
bool Factory::register_interface(std::string ifce_name, std::string ifce_desc) {
  ...
  // Return success if already registered (can happen when libramulator.so
  // is loaded by multiple shared libraries in the same process, each with
  // its own copy of the inline static registration initializers).
...
bool Factory::register_implementation(std::string ifce_name, std::string impl_name, std::string impl_desc, const Constructor_t& cstr) {
  ...
  if (auto ifce_it = m_registry.find(ifce_name); ifce_it != m_registry.end()) {
    ...
    // Return success if already registered (same shared-library duplication reason).
