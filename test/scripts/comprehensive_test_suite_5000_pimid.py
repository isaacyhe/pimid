...
Tests all memory technologies, workloads, and parameter combinations using the PIMID binary simulator
Uses the actual zsim-based pimid simulator instead of standalone C++ adapter
"""
...
RESULTS_DIR = BASE_DIR / "test/results/test_results_5000_pimid"
...
def generate_pimid_config(test_id, config):
    """Generate YAML config file for PIMID simulator"""

    mem_tech = config.get('mem_tech', 0)
    mem_tech_name = MEMORY_TECHS[mem_tech]['yaml']

    yaml_content = f"""# PIMID Test Configuration - Test {test_id:04d}
# Auto-generated for comprehensive testing

simulation:
  name: "Test_{test_id:04d}"
...
def run_test(test_id, config):
    """Run a single test configuration using PIMID binary"""
    ...
    try:
        result = subprocess.run(
            ...
            capture_output=True,
            text=True,
            timeout=300,  # 5 minute timeout per test (pimid may take longer)
            cwd=str(BASE_DIR)
        )
...
def main():
    ...
    print("Using zsim-based PIMID simulator binary")
