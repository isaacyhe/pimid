/*
 * Parallel BFS with PIM Device-Side Offloading
 * Uses MPICH for distributed computation with PIM annotations
 */
...
// BFS kernel with PIM offloading
...
int main(int argc, char *argv[]) {
    ...
    if (rank == 0) {
        printf("=== Parallel BFS with MPICH (%s) ===\n",
               USE_PIM ? "PIM Device-Side" : "Host-Side");
