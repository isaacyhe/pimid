...
#include "log.h"
#include "pin_cmd.h"
#include "stats.h"  // For StatsBackend::dump() in container exit handling
    ...
    while ((cpid = waitpid(-1, &status, WNOHANG)) > 0) {
        ...
        if (idx < MAX_THREADS) {
            ...
            if (globzinfo && !globzinfo->procExited[idx]) {
                // In container environments (e.g., gVisor), process termination signaling
                // may not work correctly. Instead of panicking, dump stats and exit gracefully.
                warn("Child %d (idx %d) exit was anomalous (container environment?), dumping stats", cpid, idx);
                // Mark process as exited to avoid further issues
                globzinfo->procExited[idx] = PROC_EXITED;
                // Force stats dump before exit
                if (globzinfo->statsBackends) {
                    info("Forcing termination stats dump due to anomalous exit");
                    globzinfo->trigger = 20000;
                    for (StatsBackend* backend : *(globzinfo->statsBackends)) {
                        backend->dump(false /*unbuffered*/);
                    }
                }
                termStatus = GRACEFUL_TERMINATION;
            }
